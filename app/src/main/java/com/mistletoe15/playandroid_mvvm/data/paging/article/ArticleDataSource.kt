 ...
 **/
class ArticleDataSource: PageKeyedDataSource<Int, HomeArticleModel>() {
    private var job by Delegates.notNull<Job>()
    //初次加载
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, HomeArticleModel>) {
    ...
    //后面一页加载
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, HomeArticleModel>) {
        job = GlobalScope.launch (Dispatchers.IO) {
            val articleList = RetrofitFactory.instance.getService(ApiService::class.java)
   ...
   //前一页加载
    override fun loadBefore(params: LoadParams<Int>, callback: LoadCallback<Int, HomeArticleModel>) {
       if(params.key > 0){
           job = GlobalScope.launch (Dispatchers.IO) {
               val articleList = RetrofitFactory.instance.getService(ApiService::class.java)
                   .getArticleListById(params.key).handled().datas
